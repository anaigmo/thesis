...
such as named graphs~\ana{cita?}, n-ary relationships~\ana{cita?}, singleton properties~(\cite{nguyen2014don}), RDF$^+$~(\cite{schueler2008querying}), and more recently, \mbox{RDF-star}~(\cite{hartig2017foundations}). This section describes popular reification approaches and shows how they can be used in RML and RML-star with a running example. 
...
There are several reification approaches for RDF as presented in \ana{section in RelWork when done}, such as standard reification, singleton properties, n-ary relationships or named graphs. 
