...
Finally, the last stage of the development process, maintenance, refers to ontology updates as new requirements are found and/or errors are fixed. The ontology presented in this work promotes the gathering of issues or new requirements through the use of issues in the ontology GitHub repository. Additionally, it provides control of changes, and the documentation enables access to previous versions. Further details are shown in \cref{sec:pub-main}.
...
%Aquí describir poquito a poquito las 4 maneras en las que se ha evaluado la ontología: con themis, oops, foops, razonador y mediante ejemplos, que en algún momento habrá que hacer
...
With these evaluations, we can conclude that the ontology is correctly encoded and implemented, and that it meets the requirements specified in \cref{sec:requirements}. 

\begin{figure}[!t]
\centering
\includegraphics[width=0.7\linewidth]{figures/CSV-extension.pdf}
...
%contar por aquí cómo se puede extender, el ejemplo con CSV, y demás, potenciales extensiones y usos. Más en concreto, criterios de separaicón entre core/extensioens (lo ampliamente usado en otros lenguajes o que forman parte del funcionamiento interno son core, lo extra o poco utilizado es extension. Explicar entonces la extensión para describir CSVs, y contar las posibles extensiones: para RDF-star, para describir casos concretos de data sources o acceso a datos, para incluir anotationes a nivel de ontologia y datos (y citar el paper que pone Ben)
...
\begin{captionedlisting}{lst:cm_sources}{Description with the Conceptual Mapping of two data sources (a JSON file and a relational database), their access and fields.}
\centering
...
\begin{captionedlisting}{lst:cm_spo}{Description with the Conceptual Mapping of the creation of regular statements from the data sources described in \cref{lst:cm_sources}.}
\centering
{\begin{lstlisting}[language=concm,firstnumber=1]
# Locations
