...
YARRRML~(\cite{Heyvaert2018yarrrml}) leverages YAML
to offer a user-friendly representation to define mapping rules, while
ShExML~(\cite{Garcia-Gonzalez2020shexml}) extends the syntax of the ShEx constraint language~(\cite{prud2014shape}).
XRM~\cite{xrm}
provides an abstract syntax that simulates programming languages and 
SMS2~(\cite{sms2}), proposed by Stardog,\footnote{\label{foot:stardog}\url{https://www.stardog.com/}} is loosely based on the SPARQL query language and extends the features of R2RML to create virtual KGs.
...
%These serialisations and translators were not compared with each other in terms of serialisations' features and system's characteristics, even though it would help to decide which serialisation fits each use case.

This section describes YARRRML-star, the extension of YARRRML to support RML-star~(\cite{delva2021rml-star}) to construct RDF-star graphs.
...
\centering
\begin{captionedlisting}{lst:chp5_csv}{Contents of the \texttt{jump-source} logical source.}
\centering
\begin{tabular}{c}
...
We extend YARRRML to specify how we can construct RDF-star graphs, aligned with the RML-star specification~\cite{iglesias2022rmlstar}.
...
All the described updates have been proposed for the YARRRML specification and is currently under review by the KG Construction Community Group\footnote{\url{https://github.com/kg-construct/yarrrml-spec/pull/4}}.
...
\cref{algorithm-yatter} presents the procedure implemented by the system to translate an input YARRRML mapping document into [R2]RML. 
...
Although YARRRML leaves the RDF-based syntax of the mapping rules to be processed only by the knowledge graph construction systems, we also provide a human-readable output considering previous experiences~(\cite{chaves2022systematic,corcho2021high,chaves2020bench}).
...
Yatter is available through PyPi as a module\footnote{\url{https://pypi.org/project/yatter/}} to be easily integrated in any Python development. % (e.g., providing RML to a KG construction system implemented in this language).
%such as SDM-RDFizer~\cite{iglesias2020sdm} or Morph-KGC~\cite{arenas2022morph}.
...
Test cases are a common method to evaluate the conformance of a system~\cite{arenas2023morph,heyvaert2019conformance}. 
%with respect to a language/syntax specification 
%(see for example). 
To the best of our knowledge, previous R2RML~\cite{boris2012r2rml} and RML~\cite{heyvaert2019conformance} test cases were not translated to any human-friendly serialisation (e.g., YARRRML).
...
%To avoid starting from scratch, we rely on the test cases proposed by R2RML~\cite{R2RML_test_cases} first, that were also extended for RML~\cite{heyvaert2019conformance}. 
...
We inspected the RML-star test cases~\cite{david_chaves_2022_6518802}, and translated to YARRRML the ones that provide a complete coverage of this extension. 
...
They are openly available\footnote{\label{foot:resources}\url{https://github.com/oeg-upm/yarrrml-validation}} to be used by any YARRRML-compliant system.
...
\subsubsection{Serialisations Comparison} %Ana

%YARRRML was developed to be a user-friendly syntax for the RML mapping language~\cite{heyvaert2018yarrrml}. There have been proposed more syntaxes with the same purpose, such as Stardog's SMS2 for R2RML and Zazuko's XRM for [R2]RML and CSVW.




We compare a set of user-friendly serialisations and languages, namely SMS2~\cite{sms2}, XRM~\cite{xrm}, ShExML~\cite{Garcia-Gonzalez2020shexml} and YARRRML~\cite{Heyvaert2018yarrrml} incorporating the updates described in \cref{sec:chp5_yarrrml-desc}, regarding their expressiveness. 
...
All serialisations can generate constant and dynamic subjects. For each set of rules, exactly one subject declaration is expected, i.e. one subject for predicate-object pairs. YARRRML can also accept no subject declaration, producing a blank node. % as default behavior.
...
\begin{table}[h!]
\caption{Features of the systems that support the different languages.}
...
\textbf{SF6} & N/A & Yes & N/A & No & Yes\\ \midrule
\textbf{SF7} & Yes & Yes & N/A & No & Yes\\ \midrule
\textbf{SF8} & Yes & N/A & N/A & No & Yes\\ \midrule
\end{tabular}
}
\end{table}
...
We also compare the systems that support the aforementioned serialisations: 
...
%\Cref{tab:chp5_eng-comparison} shows how the different systems compare to YARRRML-translator~\cite{chaves2023translator}. 
...
Additionally, based on the YARRRML test cases, we develop the corresponding test cases for the other analyzed serialisations. The results of the conformance test of the analysed systems are presented online\cref{foot:resources}.
...
XRM is installed within a coding editor, and helps actively the writing process with suggestions and warnings. YARRRML-parser supports most of the functionalities that are also implemented in Yatter but still it does not support the latest RML features. YARRRML-parser translates functions to non-standard set of RML rules, while our implementation supports the specification proposed by the W3C CG on Knowledge Graph Construction\footnote{\url{https://w3id.org/kg-construct/rml-fnml}}.


\subsubsection{Use Cases}

\ana{Igual quitar esto, salen recursos de David nada más}

We present two use cases with YARRRML-star and Yatter:% the construction of KGs for research performing organizations, and the support of mapping process for each EU member state in the Public Procurement Data Space (PPDS).

\noindent\textbf{Constructing KGs for Research-Performing Organizations.} In a previous work~\cite{chaves2022systematic} we use YARRRML-star and Yatter to support the creation of KGs for research supporting organizations with R2RML. 
Thanks to Yatter, we were able to create a fluent and iterative pipeline for testing and debugging the created mapping rules in a complex environment, where almost 2000 tables had to be mapped into RDF. 
Additionally, the easy-to-read RML outcome helped the knowledge engineer to easily identify and fix errors during the construction of the knowledge graphs.      %David

\noindent\textbf{The EU Public Procurement Data Space (PPDS).} The EU PPDS aims to construct a decentralized knowledge graph, declaratively mapping the procurement data from each EU member state into the e-Procurement Ontology (ePO)\footnote{\url{https://docs.ted.europa.eu/EPO/latest/}}. 
...
In one of the latest pilots, Spanish public procurement open data extracted from the national platform was mapped into the ePO ontology, where the mapping rules\footnote{\url{https://github.com/oeg-upm/yatter/tree/main/test/projects/PPDSTC}} contain dynamic language tags and complex XPath expression to extract the desirable data.
