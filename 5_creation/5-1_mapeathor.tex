...
Ever since the adoption of mapping languages increased, multiple  editors were developed to ease the mapping writing process and assist the needs of mapping developers. Some of the developed approaches enable editing through graphical visualization~\parencite{heyvaert2016rmleditor,sicilia2017map}, while others provide a writing environment (e.g. Ontop for Protégé\footnote{\url{https://ontop-vkg.org/tutorial/basic/setup.html\#ontop-protege-setup}}). Yet, these editors are language-oriented, as they help to create mappings in a specific language. In real world scenarios, however, different use cases require different features and implementations. As a result, several different mapping languages may need to be learnt, while the interoperability among them is very limited~\parencite{iglesias2022devising,corcho2020towards}. Moreover, graphical interfaces can result cumbersome as the number of mapping rules increase, and thus become inconvenient for complex use cases with a large set of mappings. 
